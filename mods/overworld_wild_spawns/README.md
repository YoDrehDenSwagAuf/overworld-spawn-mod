...
1. Build or download `overworld_wild_spawns-0.1.0.zip`.
...
The archive root must contain `manifest.json` and `main.lua` directly — no wrapping folder:

```text
overworld_wild_spawns-0.1.0.zip
├── manifest.json
├── main.lua
├── options.lua
├── ...
...
Build from this repo (with Gen1Recomp bootstrapped):
...
Output: `dist/overworld_wild_spawns-0.1.0.zip`
