# Release test checklist — Wilds of Kanto 1.0.0

Use this checklist before publishing the first stable GitHub release and when
verifying Mod Manager update detection.

## A. Local / CI packaging

1. Set versions consistently to `1.0.0` in `manifest.json` and `main.lua`.
2. Ensure `CHANGELOG.md` has a `## 1.0.0` section.
...
2. Publish a newer GitHub Release (for example `v1.0.0` after a `0.7.1` install).
