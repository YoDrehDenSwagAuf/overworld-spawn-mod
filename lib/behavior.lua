...
-- Optional species affinity multipliers (central config, not scattered ifs).
-- Values multiply the base weight for that behaviour.
local SPECIES_AFFINITY = {
  -- Birds / flighty: prefer idle / wander
...
function Behavior.weightsFor(species, surface, opts)
  ...
  -- Hidden cave effect may be unavailable.
...
local function canStep(map, entities, entity, player, nx, ny, region, allowLeaveHome)
  ...
  if allowLeaveHome then
    if map.isWalkableCell and not map:isWalkableCell(nx, ny) then
      -- Water tiles are often non-walkable without surfing; allow only if water surface.
...
local function lineOfSight(map, entities, x0, y0, x1, y1)
  -- Straight orthogonal LOS; walls / non-walkable break sight.
  -- Tall grass is walkable in Gen1, so isWalkableCell is the occluder test.
...
local function tryFace(entity, rng)
  ...
  bx.facing = nextFacing
  entity.facing = nextFacing
  -- Single-frame sheets ignore facing in SpriteRenderer; Entity:draw may flip.
...
local function stepToward(entity, map, entities, player, tx, ty, region, allowLeave)
  local ex, ey = entity.cellX, entity.cellY
  ...
  for _, d in ipairs(options) do
    ...
    if ok then
      if d[1] > 0 then entity.facing = "right"
      elseif d[1] < 0 then entity.facing = "left"
      elseif d[2] > 0 then entity.facing = "down"
      else entity.facing = "up" end
      entity.behaviorState.facing = entity.facing
...
function Behavior.tick(entity, ctx)
  ...
  if Behavior.isHidden(bx.behavior) then
    ...
    -- Contact handled by spawn logic on step/collision.
    return nil
  end
  ...
  if bx.behavior == Behavior.GRASS_WANDER then
    if entity.moving then
      -- Pixel step progress (simple discrete tile hops for wilds).
      entity.moveProgress = (entity.moveProgress or 0) + (ctx.dt or 0.016)
      local dur = Config.DEFAULTS.wild_step_seconds or 0.28
      if entity.moveProgress >= dur then
        entity.moving = false
    ...
    -- Choose a random in-region neighbour.
    ...
    if rng() < 0.35 then
      -- Pause / look around.
      entity.facing = dirs[rng(#dirs)][3]
      bx.facing = entity.facing
      bx.state = Behavior.STATE.PAUSED
    ...
    for _, d in ipairs(dirs) do
      ...
      if canStep(map, entities, entity, player, nx, ny, region, false) then
        entity.facing = d[3]
        bx.facing = d[3]
        entity.targetX, entity.targetY = nx, ny
        entity:setCell(nx, ny)
        entity.moving = false -- tile snap; grass overlay uses cell
  ...
  if bx.behavior == Behavior.AGGRESSIVE then
    ...
    local reactDelay = ctx.reactionDelay or Config.DEFAULTS.aggressive_reaction_delay or 0.55
    ...
    if bx.state == Behavior.STATE.CHASING or bx.chasing then
      ...
      local moved, why = stepToward(
        ...
        player.cellX, player.cellY, region, true)
      if why == "contact" then return "contact" end
      if not moved then
        bx.chaseFailCount = (bx.chaseFailCount or 0) + 1
        if bx.chaseFailCount > 24 then
          -- Give up to avoid softlock; reset to idle facing.
          bx.chasing = false
        ...
        bx.chaseFailCount = 0
        -- Pace chase steps.
        bx.nextActionAt = t + (ctx.chaseStepSeconds
          or Config.DEFAULTS.aggressive_step_seconds or 0.18)
      end
      return nil
    end

    if bx.state == Behavior.STATE.ALERT then
      if t >= (bx.alertAt or 0) then
        bx.state = Behavior.STATE.CHASING
        bx.chasing = true
        return "chase_start"
      end
